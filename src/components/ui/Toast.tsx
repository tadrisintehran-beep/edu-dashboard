...
export function Toast({ message, type, onClose }: ToastProps) {
  ...
  return (
    <div role="status" aria-live="polite" style={{
      ...
      <div onClick={onClose} role="button" tabIndex={0} aria-label="بستن پیام"
        onKeyDown={e => activateOnKey(e, onClose)}
        style={{ color: c.text, cursor: 'pointer', fontSize: '16px', opacity: 0.7 }}>✕</div>
    </div>
  )
}
